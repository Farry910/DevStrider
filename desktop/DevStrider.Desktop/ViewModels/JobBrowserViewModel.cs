...
public sealed partial class JobBrowserViewModel : ViewModelBase
{
    ...
    private readonly ManualBidStore _manualBids;
    ...
    /// <summary>The row highlighted in the queue grid.</summary>
    ...
    ///
    /// <para>
    /// A posting-level failure means the automation is finished with this link — an unreadable
    /// description, a form no adapter matches, a page behind a human gate — and retrying changes
    /// none of that. It used to stop in a Failed list, which is a pile rather than a queue: nothing
    /// worked through it, and the postings in it were usually fine and just needed a person.
    /// </para>
    ///
    /// <para>
    /// <b>Whatever the run already got goes with it.</b> Most of these fail at opening the form,
    /// which is after the description was read, so the row usually arrives on the other tab with
    /// its job description already in place and its resume one button away.
    /// </para>
    ///
    /// <para>
    /// This view-model does not know the Manual Bids tab exists. It puts the link in
    /// <see cref="ManualBidStore"/> and drops it from the queue; the other tab is listening.
    /// </para>
    /// </summary>
    private async Task HandOverToManualBidsAsync(JobLinkQueueItem item, string reason)
    {
        ...
        if (accepted)
        {
            ...
            _trace.Step("Manual", "handed to Manual Bids",
                $"{item.Url} — {reason}" + (hasJd ? "; its description went with it" : ""));
        ...
        {
            // Already on the manual list from an earlier attempt. Leave it Failed here rather than
            // dropping it silently, so the queue still shows something happened to it.
            SetStatus(item, JobLinkQueueStatuses.Failed);
            _trace.Step("Manual", "already on the manual list", item.Url);
        }
        NotifyQueueState();
    }
    ...
    public JobBrowserViewModel(SettingsService settings, ProfileContext profiles, ActivityLogService activity,
        ...
        ManualBidStore manualBids)
    {
        _settings = settings;
        _profiles = profiles;
        _activity = activity;
        _bids = bids;
        _manualBids = manualBids;
        _person = person;
    ...
    // ── links, grouped by the board they are on ─────────────────────────────
    //
    // A flat list of thirty URLs tells you nothing about which of them the automation can do. The
    // board does: every posting on one site is the same form behind the same adapter, so they
    // succeed together and they fail together. Grouping makes that visible before a run rather
    // than after — you can see six Deel links, know from the last batch that Deel defeats the
    // generic adapter, and move all six to Manual Bidding instead of paying for six failures.
    //
    // Which is why moving one link moves its whole group. Moving one and leaving its five
    // siblings queued would be choosing to fail five times on a form already known to be a
    // problem, and there is no reading of that intention worth honouring.

    /// <summary>Queued links on one job board.</summary>
    public sealed class QueueLinkGroup
    {
        public required string Site { get; init; }
        public required List<JobLinkQueueItem> Links { get; init; }

        public int Count => Links.Count;

        /// <summary>"deel.com (6)" — the count is the point, so it sits in the heading.</summary>
        public string Heading => $"{Site}  ({Count})";

        /// <summary>Whether an adapter exists for this board, which is most of whether it will work.</summary>
        public required bool HasAdapter { get; init; }

        public string Note => HasAdapter
            ? "Has a site adapter — the automatic run usually handles these."
            : "No site adapter; the generic path has to find the form. These are the ones worth moving if they have failed before.";
    }

    public ObservableCollection<QueueLinkGroup> LinkGroups { get; } = new();

    public bool HasLinkGroups => LinkGroups.Count > 0;
    public bool HasNoLinkGroups => LinkGroups.Count == 0;

    /// <summary>Rebuilds the grouping. Cheap, and the queue is small enough to redo wholesale.</summary>
    private void RebuildLinkGroups()
    {
        var groups = JobQueue
            .Where(item => item.Status == JobLinkQueueStatuses.Queued)
            .GroupBy(item => JobSiteApplyAdapters.SiteNameFor(item.Url), StringComparer.OrdinalIgnoreCase)
            .OrderByDescending(group => group.Count())
            .ThenBy(group => group.Key, StringComparer.OrdinalIgnoreCase)
            .Select(group =>
            {
                var first = group.First();
                var hasAdapter = Uri.TryCreate(first.Url, UriKind.Absolute, out var uri)
                                 && !ReferenceEquals(JobSiteApplyAdapters.Resolve(uri),
                                                     JobSiteApplyAdapters.Default);
                return new QueueLinkGroup
                {
                    Site = group.Key.Length > 0 ? group.Key : "(unknown)",
                    Links = group.ToList(),
        ...
        LinkGroups.Clear();
        foreach (var group in groups) LinkGroups.Add(group);
        OnPropertyChanged(nameof(HasLinkGroups));
        OnPropertyChanged(nameof(HasNoLinkGroups));
    }

    /// <summary>
    /// Moves every queued link on one board to Manual Bidding.
    ///
    /// <para>
    /// The whole group, always. These are one form behind one adapter; a link that will defeat the
    /// automation has siblings that will defeat it the same way, and moving one of six is choosing
    /// to spend five more failures finding that out.
    /// </para>
    /// </summary>
    [RelayCommand]
    private async Task MoveGroupToManualAsync(QueueLinkGroup? group)
    {
        if (group == null || group.Links.Count == 0) return;
        if (IsAutomaticQueueRunning)
        {
            StatusMessage = "Stop the run before moving links — one of these may be the link it is on.";
            return;
        }

        var moved = 0;
        foreach (var item in group.Links.ToList())
        {
            if (item.Status != JobLinkQueueStatuses.Queued) continue;
            if (!await _manualBids.AddAsync(item)) continue;
            JobQueue.Remove(item);
            moved++;
        }
        ...
        _trace.Step("Queue", $"moved {moved} {group.Site} link(s) to Manual Bidding",
            "same board, same form, same outcome");
        _activity.Info("Auto bidding", $"{moved} {group.Site} link(s) moved to Manual Bidding",
            "They are one form behind one adapter, so they were moved together.");
        StatusMessage = $"Moved all {moved} {group.Site} link(s) to Manual Bidding.";
    }

    /// <summary>
    /// Moves one link's whole group. The per-link button exists because that is where the decision
    /// gets made — you recognise a posting you know will fail — but it acts on the group, and the
    /// button says so.
    /// </summary>
    [RelayCommand]
    private Task MoveLinkGroupToManualAsync(JobLinkQueueItem? item)
    {
        if (item == null) return Task.CompletedTask;
        var site = JobSiteApplyAdapters.SiteNameFor(item.Url);
        var group = LinkGroups.FirstOrDefault(g => g.Site.Equals(site, StringComparison.OrdinalIgnoreCase));
        return group == null ? Task.CompletedTask : MoveGroupToManualAsync(group);
    }
        ...
        FailureScope scope = FailureScope.Machinery)
    {
        ...
        else
        {
            ...
            await HandOverToManualBidsAsync(item, message);
        }
        await SaveQueueAsync();
    ...
    private void NotifyQueueState()
    {
        ...
        // The grouping is a view over the queue, so it is rebuilt wherever the queue is announced.
        RebuildLinkGroups();
        NotifyJdState();
        NotifyCodeState();
    }
